...
def weekly_movieDetail_S3_Redshift():
    ...
    def get_weekly_movie_list(target_dt):
        ...
        data=response.json()   
        movie_cds=[movie["movieCd"] for movie in data["boxOfficeResult"]['weeklyBoxOfficeList']]
...
#4시작
    exec_date = "{{ data_interval_end.subtract(days=7).strftime('%Y%m%d') }}"
    ...
    weekly_list = get_weekly_movie_list(target_dt=exec_date)
    
    
    new_movies = find_new_movies(movie_cds=weekly_list)
...
weekly_movieDetail_S3_Redshift()
